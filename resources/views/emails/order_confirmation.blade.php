...
<div class="wrapper">
    ...
    <div class="body">
        <p style="margin:0 0 24px;font-size:.95rem;color:#1a2e1a">
            Hi <strong>{{ $order->shipping_name }}</strong>, your organic order has been placed successfully.
            We'll get it ready for you soon!
        </p>

        <div class="order-id">
            <div class="lbl">Order Number</div>
            <div class="num">#{{ str_pad($order->id, 6, '0', STR_PAD_LEFT) }}</div>
        </div>
        ...
        <div class="totals">
            ...
            <div class="total-row">
                <span>Subtotal</span>
                <span>PKR {{ number_format($order->total + $order->discount, 0) }}</span>
            </div>
            <div class="total-row" style="color:#e9a825">
                <span>Discount ({{ $order->coupon_code }})</span>
                <span>−PKR {{ number_format($order->discount, 0) }}</span>
            </div>
            @endif
