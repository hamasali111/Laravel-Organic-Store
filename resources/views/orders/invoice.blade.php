    ...
    <div class="page">
        <div class="header">
            ...
            <div class="invoice-meta">
                ...
                <p style="margin-top:6px">Date: {{ $order->created_at->format('M d, Y') }}</p>
                <p>
                    <span class="badge status-{{ $order->status }}">{{ $order->statusLabel() }}</span>
                </p>
            </div>
        </div>
        ...
        <div class="grid2">
            ...
            <div class="col">
                ...
                <p>
                    ...
                    <strong>Status:</strong> {{ $order->paymentStatusLabel() }}<br>
        ...
        <table class="totals-table">
            <tr>
                ...
                <td style="text-align:right">PKR
                    {{ number_format($order->total - ($order->shipping_fee ?? 0) + ($order->discount ?? 0), 0) }}</td>
            </tr>
            @if ($order->discount > 0)
                <tr>
                    <td style="color:#2d6a4f">Discount ({{ $order->coupon_code }})</td>
                    <td style="text-align:right;color:#2d6a4f">-PKR {{ number_format($order->discount, 0) }}</td>
                </tr>
            @endif
            <tr>
                <td style="color:#666">Shipping</td>
                <td style="text-align:right">
                    {{ ($order->shipping_fee ?? 0) == 0 ? 'Free' : 'PKR ' . number_format($order->shipping_fee, 0) }}
                </td>
            </tr>
            <tr class="grand-total">
                <td>Grand Total</td>
                <td style="text-align:right">PKR {{ number_format($order->total, 0) }}</td>
            </tr>
        </table>
