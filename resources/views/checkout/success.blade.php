...
@section('title', 'Order Confirmed — Organic Store')
...
<div class="success-wrap">
    <div class="success-card">
        ...
        <p>
            Thank you, <strong>{{ $order->shipping_name }}</strong>! Your organic order has been placed successfully.
        </p>

        {{-- Mini shipping tracker --}}
        <div class="mini-tracker">
            ...
            @foreach($steps as $i => $s)
                @if($i > 0)<div class="mt-line {{ $i <= $current ? 'done' : '' }}"></div>@endif
                <div class="mt-step">
                    <div class="mt-dot {{ $i > $current ? 'inactive' : '' }}">{{ $i < $current ? '✓' : $s['icon'] }}</div>
                    <div class="mt-label {{ $i <= $current ? 'done' : '' }}">{{ $s['label'] }}</div>
                </div>
            @endforeach
        </div>

        <div class="email-notice">
            📧 A confirmation email has been sent to <strong>{{ $order->shipping_email }}</strong>. Check your inbox (or spam) for your order details.
        </div>
        ...
        <div class="order-details">
            ...
            <div class="order-detail-row">
                <span>Status</span>
                <span style="color:var(--green)">{{ $order->statusLabel() }}</span>
            </div>
            <div class="order-detail-row">
                <span>Shipping to</span>
                <span>{{ $order->shipping_city }}, {{ $order->shipping_zip }}</span>
            </div>
            @if($order->discount > 0)
            <div class="order-detail-row">
                <span>Discount</span>
                <span style="color:var(--amber)">−PKR {{ number_format($order->discount, 0) }}</span>
            </div>
            @endif
            <div class="order-detail-row">
                <span>Order Total</span>
                <span>PKR {{ number_format($order->total, 0) }}</span>
            </div>
            <div class="order-detail-row">
                <span>Items</span>
