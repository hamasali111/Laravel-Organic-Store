@extends('layouts.app')
@section('title', 'Your Cart — Organic_store')
@push('styles')
    <style>
        .cart-layout {
            max-width: 1000px;
...
@endpush
@section('content')
    <div class="breadcrumb"><a href="{{ route('home') }}">Home</a> <span>›</span> Your Cart</div>
                    ...
                    <tbody>
                        @foreach ($items as $item)
                            <tr class="cart-row">
                                ...
                                <td style="font-weight:600">PKR {{ number_format($item->product->price, 0) }}</td>
                                <td>
            ...
            <div class="order-summary">
                ...
                @if ($discount > 0)
                    <div class="summary-row" style="color:var(--green)"><span>🎟 Coupon
                            ({{ $coupon }})</span><span>-PKR {{ number_format($discount, 0) }}</span></div>
                @endif
                <div class="summary-row"><span>Shipping</span><span>{{ $total >= 5000 ? 'Free 🎉' : 'PKR 200' }}</span>
                </div>
                @if ($total < 5000)
                    <p
                ...
                <div class="summary-row total">
                    <span>Total</span>
                    <span>PKR {{ number_format($total + ($total >= 5000 ? 0 : 200), 0) }}</span>
                </div>

                <div class="coupon-box">
                    @if ($coupon)
                        <div class="coupon-applied">
                            <span style="font-size:.84rem;font-weight:600;color:var(--green)">🎟 {{ $coupon }}
                                applied!</span>
                    ...
                    @else
                        <form method="POST" action="{{ route('cart.coupon') }}" style="display:flex;gap:8px">
                            @csrf
